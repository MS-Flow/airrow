...
- **ADR-0001** — Arrow runs on Next.js App Router (dogfoods the golden path; one codebase for marketing/app/server).
...
Product-level: Arrow generates foundations, never applications (Product Constitution §1). Specs are the source of truth everywhere (§3). Dark mode first (§7).
