...
const DB_URL =
  ...
  "postgresql://postgres:postgres@90.235.63.115/postgres";
...
describe.skipIf(!dbUp)(
  "handle_new_user signup trigger (local Supabase)",
  () => {
    ...
    afterAll(async () => {
      await db.query(
        "delete from public.organizations where created_by = any($1)",
        [[userId, userIdB]],
      );
      await deleteByEmail(EMAIL);
      await deleteByEmail(EMAIL_B);
