...
const BEYOND_THE_PAGE = [
  ...
  "The commands that touch anything run on the founder's own machine, inside the repository they downloaded, and only when they run them. For a new project that is /start, which sets the project up until it runs and then builds the product's core action, once. A project that already exists gets /sync, which reads the code that is there and rewrites the foundation's documents to match — it changes no code and deletes nothing. Alongside it, if the founder asks for it during the interview, /cleanup: it reorganises their own files so the structure reads, proposes what nothing uses and deletes only what they approve, stages everything and commits nothing, and reports what the project is still tied to rather than removing it. A foundation ships one of those sets, decided by where the project came from and what the founder chose.",
