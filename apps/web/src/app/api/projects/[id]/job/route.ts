...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  let job = await latestJob(id);
  if (job && job.status === "running") {
    const age = Date.now() - new Date(job.heartbeatAt).getTime();
    if (age > 60_000) {
      // Dev-server restart mid-job (F-401 risk): mark failed, offer retry.
      await updateJob(job.id, { status: "failed", error: "Generation was interrupted. Retry to regenerate." });
      await setProjectStatus(id, "failed");
      job = await latestJob(id);
    }
  }
  const current = await getProject(session.org.id, id);
  return NextResponse.json({ job, projectStatus: current?.status ?? null });
}
