...
- **Archer** is mounted once, in `app/(public)/layout.tsx`, and therefore renders on every public page
  and on no `/app` page — that tree is outside the route group. Pages never import `ChatWidget`
  themselves. When it cannot answer, or someone asks for a person, it points at `/app/support` and says
  the sign-in step first (spec 158).
